...
CEffect_fly_fire::CEffect_fly_fire(CSprite* _sprite){
    mLifeTime = 9999;
    mAttacher = Global::spriteList->GetSharedPtr(_sprite);
}

void CEffect_fly_fire::Update(float _deltaTime) {
...
void CEffect_fly_fire::Draw(CCamera& _camera) {
    if(IsDead())return;
